...
function useScrollReveal() {
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          // Removes class to enable repeating scroll animation
    ...
    const applyReveal = () => {
      const selectors = '.card, .kpi-card, .insight-card, .table-wrapper, .section-title';
    ...
    // Observe dynamic changes in the DOM to attach observer to newly rendered components
    const mo = new MutationObserver(applyReveal);
    const appShell = document.querySelector('.app-shell');
    if (appShell) mo.observe(appShell, { childList: true, subtree: true });
